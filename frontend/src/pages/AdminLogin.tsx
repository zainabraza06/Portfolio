  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md glass-card p-8" style={{ animation: 'fadeInUp 0.5s ease both' }}>
        ...
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-1.5 text-muted hover:text-accent text-sm mb-6 transition-colors"
        >
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-accent flex items-center justify-center text-accent-ink font-bold text-xl mx-auto mb-4 shadow-lg"
            style={{ boxShadow: '0 0 30px rgba(32,178,166,0.4)' }}>
            ZR
          </div>
          <h1 className="text-2xl font-bold text-text font-serif">Admin Portal</h1>
          <p className="text-muted text-sm mt-1">Zainab Raza Malik · Portfolio</p>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="admin-email" className="block text-xs font-semibold text-muted mb-1.5 uppercase tracking-wide">Email</label>
            <input
              id="admin-email"
              type="email"
              required
          ...
          <div>
            <label htmlFor="admin-password" className="block text-xs font-semibold text-muted mb-1.5 uppercase tracking-wide">Password</label>
          ...
          {error && (
            <div className="flex items-center gap-2 p-3 rounded-xl bg-bad/10 border border-bad/30 text-bad text-sm">
              <span>❌</span> {error}
            </div>
          )}
        ...
        <p className="text-center text-xs text-muted mt-6">
          Default credentials in <code className="text-accent">backend/.env</code>
        </p>
      </div>
    </div>
  );
